package com.lifeteam.farbeat.demo.wheelpicker.ex;
...
import com.lifeteam.farbeat.demo.wheelpicker.listener.IWheelEntity;
...
public class WheelView<T> extends View implements Runnable {
     ...
     * @param attrs   attrs
     */
    private void initAttrsAndDefault(@NotNull Context context, AttributeSet attrs) {
     ...
     * @param context 上下文
     */
    private void initDefaultVolume(@NotNull Context context) {
     ...
     * @param centerToBaselineY 文字中心距离baseline的距离
     */
    private void clipAndDraw3DText(@NotNull Canvas canvas, String text, int clipTop, int clipBottom, float rotateX, float offsetY, float offsetZ, int centerToBaselineY) {
        canvas.save();
        canvas.clipRect(mClipLeft, clipTop, mClipRight, clipBottom);
    ...
    public void setSelectedItemPosition(int position, boolean isSmoothScroll, int smoothDuration) {
        ...
        if (isSmoothScroll) {
            ...
            mScroller.startScroll(0, mScrollOffsetY, 0, itemDistance, smoothDuration > 0 ? smoothDuration : DEFAULT_SCROLL_DURATION);
            invalidateIfYChanged();
            ViewCompat.postOnAnimation(this, this);
